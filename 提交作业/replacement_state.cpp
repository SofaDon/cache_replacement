    ...
    UINT32 tid, Addr_t PC, UINT32 accessType, bool cacheHit )
{
    ...
    {
        UpdateLFU( setIndex, updateWayID, cacheHit );
    }
    else if( replPolicy == CRC_REPL_CLOCK )
    {
...
INT32 CACHE_REPLACEMENT_STATE::Get_LFU_Victim( UINT32 setIndex )
{
    ...
    // search smallest count
    for(UINT32 way = 0; way < assoc; way++) 
    {
        if( replSet[way].LFUcount < replSet[LFUway].LFUcount ) 
        {
            LFUway = way;
        }
    }
...
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
void CACHE_REPLACEMENT_STATE::UpdateLFU( UINT32 setIndex, INT32 updateWayID, bool cacheHit )
{
    if (cacheHit)
    {
        repl[setIndex][updateWayID].LFUcount++;
    }
    else
    {
        repl[setIndex][updateWayID].LFUcount = 1;
    }
